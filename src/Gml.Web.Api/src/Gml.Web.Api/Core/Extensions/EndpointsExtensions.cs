...
public static class EndpointsExtensions
{
    public static WebApplication RegisterEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/v1/filesystem/directory", FileSystemHandler.GetRootDirectory)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Получение информации о корневой директории";
                return generatedOperation;
            })
            .WithDescription("Получение информации о корневой директории лаунчера")
            .WithName("FileSystem_GetRootDirectory")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapGet("/api/v1/filesystem/directory/{*path}", FileSystemHandler.GetDirectoryContents)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Получение содержимого директории";
                return generatedOperation;
            })
            .WithDescription("Получение списка файлов и папок в указанной директории")
            .WithName("FileSystem_GetDirectoryContents")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapPost("/api/v1/filesystem/archive/multiple", FileSystemHandler.CreateArchiveMultiple)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Создание архива из нескольких файлов или директорий";
                return generatedOperation;
            })
            .WithDescription("Создание ZIP архива из нескольких указанных путей")
            .WithName("FileSystem_CreateArchiveMultiple")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapPost("/api/v1/filesystem/archive/{*path}", FileSystemHandler.CreateArchive)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Создание архива из файла или директории";
                return generatedOperation;
            })
            .WithDescription("Создание ZIP архива из указанного пути (файла или директории)")
            .WithName("FileSystem_CreateArchiveSingle")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapPost("/api/v1/filesystem/extract", FileSystemHandler.ExtractArchive)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Распаковка архива";
                return generatedOperation;
            })
            .WithDescription("Распаковка ZIP архива с заменой существующих файлов и созданием отсутствующих папок")
            .WithName("FileSystem_ExtractArchive")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapGet("/api/v1/filesystem/download/{*path}", FileSystemHandler.DownloadFile)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Скачивание файла";
                return generatedOperation;
            })
            .WithDescription("Скачивание файла по указанному пути")
            .WithName("FileSystem_DownloadFile")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapPost("/api/v1/filesystem/upload/{*path}", FileSystemHandler.UploadFile)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Загрузка файла";
                return generatedOperation;
            })
            .WithDescription("Загрузка файла в указанную директорию")
            .WithName("FileSystem_UploadFile")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .DisableAntiforgery()
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapDelete("/api/v1/filesystem/files", FileSystemHandler.DeleteFiles)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Удаление нескольких файлов";
                return generatedOperation;
            })
            .WithDescription("Удаление нескольких файлов по указанным путям")
            .WithName("FileSystem_DeleteFiles")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapDelete("/api/v1/filesystem/directories", FileSystemHandler.DeleteDirectories)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Удаление нескольких директорий";
                return generatedOperation;
            })
            .WithDescription("Удаление нескольких директорий по указанным путям")
            .WithName("FileSystem_DeleteDirectories")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapDelete("/api/v1/filesystem/file/{*path}", FileSystemHandler.DeleteFile)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Удаление файла";
                return generatedOperation;
            })
            .WithDescription("Удаление файла по указанному пути")
            .WithName("FileSystem_DeleteFile")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapDelete("/api/v1/filesystem/directory/{*path}", FileSystemHandler.DeleteDirectory)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Удаление директории";
                return generatedOperation;
            })
            .WithDescription("Удаление директории по указанному пути")
            .WithName("FileSystem_DeleteDirectory")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapGet("/api/v1/filesystem/file/{*path}", FileSystemHandler.ReadFileContent)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Чтение содержимого файла";
                return generatedOperation;
            })
            .WithDescription("Получение содержимого файла для редактирования")
            .WithName("FileSystem_ReadFileContent")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapPost("/api/v1/filesystem/file", FileSystemHandler.SaveFileContent)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Сохранение содержимого файла";
                return generatedOperation;
            })
            .WithDescription("Сохранение отредактированного содержимого файла")
            .WithName("FileSystem_SaveFileContent")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapPost("/api/v1/filesystem/directory/create/{*path}", FileSystemHandler.CreateDirectory)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Создание новой директории";
                return generatedOperation;
            })
            .WithDescription("Создание новой директории по указанному пути")
            .WithName("FileSystem_CreateDirectory")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));

        app.MapPost("/api/v1/filesystem/file/create/{*path}", FileSystemHandler.CreateFile)
            .WithOpenApi(generatedOperation =>
            {
                generatedOperation.Summary = "Создание нового файла";
                return generatedOperation;
            })
            .WithDescription("Создание нового пустого файла по указанному пути")
            .WithName("FileSystem_CreateFile")
            .WithTags("FileSystem")
            .Produces<ResponseMessage>((int)HttpStatusCode.NotFound)
            .RequireAuthorization(c => c.RequireRole("Admin"));
